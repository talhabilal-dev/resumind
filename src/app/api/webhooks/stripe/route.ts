...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      // Integrity check: credits must match the advertised per-credit price
      // ($0.10/credit). Rejects mismatched metadata before crediting.
      if (amountCents !== credits * 10) {
        console.error("Stripe webhook: price/credit mismatch ignored", {
      ...
      // Atomically claim the transition pending/missing -> completed so that
      // concurrent webhook deliveries can never double-credit.
      let claimed: any = null;
      try {
        claimed = await PaymentModel.findOneAndUpdate(
          { stripePaymentId: session.id, status: { $ne: "completed" } },
          {
            $set: {
              userId,
        ...
        // Unique index on stripePaymentId: a concurrent delivery already
        // inserted the completed record, so this is a duplicate event.
